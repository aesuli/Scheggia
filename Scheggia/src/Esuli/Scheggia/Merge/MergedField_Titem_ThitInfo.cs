...
namespace Esuli.Scheggia.Merge
{
    ...
    public class MergedField<Titem, Tcomparer, Thit>
        ...
        where Thit : IComparable<Thit>
    {
        ...
        public MergedField(string name, List<KeyValuePair<int, IField>> fieldList, int[] idShifts)
        {
            this.name = name;
            ...
            while (lexiconMerger.MoveNext())
            {
                ...
                var sourceLexiconItems = new List<KeyValuePair<int, int>>();
        ...
        public IPostingEnumerator<Thit> GetSpecializedPostingEnumerator(int enumeratorId, int postingId)
        {
            ...
            {
                postingEnumerators[i] = OffsetPostingEnumerator<Thit>.Build(idShifts[postingIdPair.Key],
